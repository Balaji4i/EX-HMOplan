...
public class ComplaintsFormBB {
    ...
    private boolean callSubmitPayload() {
        ViewObject mediVo = com.view
                               .uiutils
                               .ADFUtils
                               .findIterator("HMOComplaint_VOIterator")
                               .getViewObject();
        ...
        String dateIssueOccurred =
            r.getAttribute("DateIssueOccurred") != null ? r.getAttribute("DateIssueOccurred").toString() : "";
        ...
        String xmlData =
            ApprovalPayload.complaintChangesXMLData(complaintId, year, hmo_ref, personId, personNo, title, first_name,
                                                    last_name, email, hmoOption, stateOfResi, hospital, complaint,
                                                    dateIssueOccurred, reqType, comments, status, complaintReqNo);
        System.err.println("xmlData =>" + xmlData);
        a = ApprovalProcess.invokeWsdl(xmlData, ApprovalPayload.COMPLAINT_WSDL,
                                       ApprovalPayload.COMPLAINT_CHANGE_METHOD);
        if (a[0] != null && a[0].equals("True")) {
            ADFUtils.findOperation("Commit").execute();
            com.view
               .utils
               .JSFUtils
               .addFacesInformationMessage("Complaint Form Submitted Successfully !!");
            return true;
        } else {
