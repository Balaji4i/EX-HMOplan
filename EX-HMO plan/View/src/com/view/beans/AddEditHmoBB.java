...
public class AddEditHmoBB {
    ...
    private RichPopup exempattachPop;
    ...
    private boolean isAdditionalAttachmentAdded(String requestNumber) {
        
        System.out.println("requestNumber---- " + requestNumber);
        
        ViewObject LineVo =
            com.view.utils.ADFUtils.getApplicationModuleForDataControl("Oando_AMDataControl").findViewObject("ExEmpAttachment_EOView");
        ViewCriteria viewCriteria = LineVo.createViewCriteria();
        ViewCriteriaRow viewCriteriaRow = viewCriteria.createViewCriteriaRow();
        viewCriteriaRow.setAttribute("SourceDocumentId", requestNumber);
        viewCriteria.addRow(viewCriteriaRow);
        LineVo.applyViewCriteria(viewCriteria);
        LineVo.executeQuery();
        
        System.out.println(LineVo.getEstimatedRowCount() + "----est row count");
...
//            Row r = itr.next();
//            String count =
//                r.getAttribute("AttachmentCount") != null ? r.getAttribute("AttachmentCount").toString() : "0";
//            System.out.println("AttachmentCount" + count);
//            if ("0".equals(count)) {
//                return false;
//            }
//        }
//        itr.closeRowSetIterator();
    ...
    public String onSubmitHMOReq() {
        ...
        if (xxhrHmoHdrRow != null) {
            System.out.println("Header ID " + xxhrHmoHdrRow.getHmoRequestNo().toString());
            String requestNumber = xxhrHmoHdrRow.getHmoRequestNo().toString();
            boolean attachedFile = isAdditionalAttachmentAdded(requestNumber);
            System.out.println("Boolean Value --- " + attachedFile);
            if (!attachedFile) {
                System.out.println("inside if loop");
                JSFUtils.addFacesErrorMessage("Please Check, Attach Payment Details is Missing!");
                return null;
            }
    ...
    public void onChangeDepType(ValueChangeEvent valueChangeEvent) {
        ...
        if ("Employee +1 Dependant Child".equals(hmoOption)) {
            Row[] dtlRows = dtlVo.getFilteredRows("DependentType", "Spouse");
            if (dtlRows.length > 1) {
                RichSelectOneChoice depType = (RichSelectOneChoice) JSFUtils.findComponentInRoot("soc1");
                depType.setValue(" ");
                AdfFacesContext.getCurrentInstance().addPartialTarget(depType);
    ...
    public void setEmployeeHMOCost() {
        ...
        String dob = vo.getCurrentRow().getAttribute("EmpDob") == null ? "" : vo.getCurrentRow()
                                                                                           .getAttribute("EmpDob")
                                                                                           .toString();
    ...
    // public void onClickClosePopup(ActionEvent actionEvent) {
    ...
    public void onClickDelete(ActionEvent actionEvent) {
        com.view
           .utils
           .ADFUtils
           .findOperation("DeleteExEmpAttach")
           .execute();
        com.view
           .utils
           .ADFUtils
    ...
    public void downloadFileListener_Self(FacesContext facesContext, OutputStream outputStream) throws IOException {

        ViewObject hdrVo = ADFUtils.findIterator("ExEmpAttachment_EOView1Iterator").getViewObject();
        Row row = hdrVo.getCurrentRow();
        BlobDomain blob = (BlobDomain) row.getAttribute("Attachment");
        try {
            IOUtils.copy(blob.getInputStream(), outputStream);
    ...
    public void uploadFileVCE(ValueChangeEvent vce) {
        if (vce.getNewValue() != null) {
            UploadedFile myfile = (UploadedFile) vce.getNewValue();
            if (myfile != null) {
                ViewObject vo = ADFUtils.findIterator("ExEmpAttachment_EOView1Iterator").getViewObject();
                Row curRow = vo.getCurrentRow();
                String filename = myfile.getFilename();
                String ContentType = myfile.getContentType();
                System.err.println("AttachmentType" + ContentType);
    ...
    //        public String UploadFileActionToDB(UploadedFile file, String type) {
    ...
    //              (DCIteratorBinding) getBindingsCont().get("ExEmpAttachment_EOView1Iterator");
    //                    ViewObject vo = imageIter.getViewObject();
    //                     ExEmpAttachment_EOViewRowImpl curRow = (ExEmpAttachment_EOViewRowImpl) vo.getCurrentRow();
    //
    //                    try {
    ...
    //            return null;
    //        }
    public void setExempattachPop(RichPopup exempattachPop) {
        this.exempattachPop = exempattachPop;
    }

    public RichPopup getExempattachPop() {
        return exempattachPop;
    }
